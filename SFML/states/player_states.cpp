...
void PlayerState::jump(Player* player)
{
    if (player->djump == 0) {
        ...
        startFalling(player);
        player->djump = 1;
    } else if (player->djump == 1) {
        player->mPosition.y -= 1;
        player->mVelocity.y = -kJumpingVelocity * 0.75;
        startFalling(player);
        player->djump = 2;
    }
}
...
FirstAttack::FirstAttack(Player* player) : PlayerState()
{
    ...
    mAnimation.addTextureRect({ 64, 221, 35, 45});
    mAnimation.addTextureRect({114, 221, 35, 45});
    mAnimation.addTextureRect({164, 221, 35, 45});
    mAnimation.addTextureRect({214, 221, 35, 45});
    mAnimation.addTextureRect({264, 221, 35, 45});
