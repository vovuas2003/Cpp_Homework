...
uint64_t getMaxPar(int n, const std::vector<uint64_t>& v)
{
    ...
    std::vector<std::future<void>> threads(n - 1);
    ...
    for(unsigned long i = 0; i < (n - 1); ++i)
    {
        ...
        threads[i] = std::async(std::launch::async, getMaxOne, block_start, block_end, std::ref(results[i]));
        block_start = block_end;
    }
    getMaxOne(block_start, v.end(), results[n - 1]);
    std::for_each(threads.begin(), threads.end(), std::mem_fn(&std::future<void>::get));
